...
**Estado del documento: APROBADO, con un ajuste puntual antes de consolidarse como precedente.** La versión original de §5 y §8 autorizaba, de forma general, que `recorridos_sec` llevara corrientes contextuales bajo `sin_principal_clasificable`, por analogía con `frontera_constitutiva` (`schwartz_seaofstorms`). Dirección Académica retira esa autorización general: la categoría nueva debe resolver únicamente la ausencia demostrada de principal, sin modificar de paso el alcance ya consolidado de `recorridos_sec`. La regla queda, en consecuencia, más estricta que en la versión original — ver §5 y §8 revisados.
...
Cuando `arquitectura_recorrido.tipo` es `sin_principal_clasificable`, el resto del bloque queda fijado, de forma provisional y estricta, así: `recorrido: null` (autorizado por la extensión del §9, ver §7); **`recorridos_sec: []` sin excepción**; `estado_recorrido: "confirmado"` sin excepción; `fuente_recorrido` no vacío.

Se descarta, por ahora, la autorización general que una versión anterior de esta decisión daba a `recorridos_sec` bajo esta categoría (por analogía con `frontera_constitutiva`, donde sí conviven principal-ausente y secundarias — `schwartz_seaofstorms` es frontera C6/C7 con dos secundarias). La analogía no se sostiene: en frontera constitutiva las secundarias se declaran junto a una arquitectura que sigue siendo relacional entre corrientes; en `sin_principal_clasificable` no hay ninguna corriente estructurante que sirva de ancla a una secundaria — autorizar `recorridos_sec` aquí habría alterado, de paso y sin evidencia propia, el significado ya consolidado de "recorrido secundario" (§10 del contrato: "dimensión sustantiva del argumento... subordinada a la arquitectura interpretativa definida por el recorrido principal" — pero aquí no hay recorrido principal al cual subordinarse). La nueva categoría resuelve exclusivamente la ausencia demostrada de principal; no decide, de paso, si puede haber secundarias sin principal. Si un caso futuro demuestra esa necesidad con evidencia propia, se decidirá mediante una decisión normativa separada — no por extensión silenciosa de esta.
...
- `sin_principal_clasificable` exige además, específicamente: `recorridos` vacío, `recorrido: null`, **`recorridos_sec: []`**, `estado_recorrido: "confirmado"` y `fuente_recorrido` no vacío — las cinco condiciones que impiden que la categoría se use como atajo de una lectura incompleta o como vía para extender de paso el significado de `recorridos_sec`.
