...
**Mecanismo de despliegue.** Netlify, sitio `4d684b99-a19d-4952-b435-eb84cd3d5691`, dominio propio declarado en `CNAME` (`pensamientocaribe.org`).

> **CORRECCIÓN, 17 de septiembre de 2026.** Este párrafo es incorrecto y no se reescribe, para dejar registro del error. El sitio **no se despliega con Netlify: lo sirve GitHub Pages** desde la rama `main`. Comprobado con `curl -I https://pensamientocaribe.org/`, que responde `server: GitHub.com` y `x-github-request-id`, más el `CNAME` del repositorio y la ausencia de rama `gh-pages`. El mecanismo se dedujo aquí del `siteId` que guarda `.netlify/state.json`, que es lo que la incidencia 2 de este mismo informe ya señalaba como resto inerte de una prueba vieja. Esa deducción propagó el error a otros documentos y memorias del proyecto. La carpeta `.netlify/` quedó retirada del repositorio y añadida a `.gitignore` en esta misma fecha, por decisión de la Dirección Académica. No hay `netlify.toml` en la raíz del repositorio, de modo que la configuración efectiva vive en el panel de Netlify. El repositorio ya tiene su propia puerta de publicación, `scripts/publicar.sh`, que corre las comprobaciones y solo entonces commitea y empuja.
