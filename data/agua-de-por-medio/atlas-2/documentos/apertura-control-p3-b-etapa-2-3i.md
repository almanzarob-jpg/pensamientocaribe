...
| `sharpewake` | `RESERVA_CONCEPTUAL` (no cuestiona la clasificación) | El control semántico final (§10 de `decision-academica-sharpewake-p3-etapa-2-3j.md`, contrastado contra `dictamen-academico-c2.md` §6-7.3 y `dictamen-academico-c9.md` §3-4 y §10) muestra que C2 representa de forma directa el régimen esclavista histórico y C9 representa de forma directa la transmisión y el archivo vivo, pero que ninguna de las dos definiciones vigentes, ni la regla oficial de frontera C2/C9 (Decisión 12, alternancia principal/secundaria), nombra la relación temporal/ontológica específica que Sharpe articula: la esclavitud como singularidad todavía activa que constituye el presente negro, dentro de la cual `wake work` produce vigilancia, cuidado, conocimiento e imaginación de vida | `FRONTERA_CONSTITUTIVA(C2,C9)`, `recorridos_sec: []`, `estado_recorrido: "confirmado"` — sin cambios | Ninguna sobre la implementación; a diferencia de `rebeldestiny`, aquí la clasificación es plenamente válida y la reserva se añade como excedente, no como sustituto de un principal inexistente | Estudio comparativo de otros casos del corpus cuyo argumento articule una relación temporal/ontológica equivalente entre dos corrientes en frontera | No crear una corriente nueva (C11) a partir de este único caso; no modificar las definiciones vigentes de C2 o C9 a partir de este caso; no reclasificar por analogía otros casos de frontera C2/C9 sin lectura y prueba propias |
...
**Fuente (sharpewake):** `decision-academica-sharpewake-p3-etapa-2-3j.md` §9 y §10.

| `dubuissonhaiti` | `SIN_PRINCIPAL_CLASIFICABLE` con dimensiones parciales (extensión normativa nueva, ver `decision-normativa-sin-principal-con-dimensiones-p3-etapa-2-3j.md`) | El vocabulario C1–C10 no captura el principio transversal mediante el cual *Reclaiming Haiti's Futures* articula fracturas coloniales/institucionales, precariedad e improvisación/rasanblaj/imaginación radical hacia la producción de lugar y de futuro habitables (placemaking / inhabitability / homing / fractura-sutura). C5 y C9 organizan segmentos demostrables del libro (capítulos 1–2 y 4–5 respectivamente) sin que ninguna organice el argumento completo — a diferencia de `rebeldestiny`, aquí sí hay dimensiones parciales que la implementación debe conservar, no solo un principio transversal sin corriente candidata | `recorrido: null`, `recorridos_sec: ["c5","c9"]`, `arquitectura_recorrido: {"tipo":"sin_principal_clasificable","recorridos":[]}`, `estado_recorrido: "confirmado"` | Ninguna — la ausencia de principal y la conservación de C5/C9 como dimensiones sustantivas parciales son la conclusión académica misma, no una limitación de la implementación | Acumulación de evidencia comparativa de otros casos del corpus que documenten una tensión equivalente entre el vocabulario C1–C10 y un principio organizador transversal que conserve dimensiones parciales demostradas | No crear una corriente nueva (C11) a partir de este único caso; no reclasificar por analogía otros casos del corpus sin lectura y prueba de retirada propias; no usar `recorridos_sec` bajo `sin_principal_clasificable` para casos futuros sin documentar individualmente, para cada corriente, sustantividad segmentaria, retirada segmentaria destructiva y retirada global no destructiva |

**Fuente (dubuissonhaiti):** `lectura-dubuissonhaiti-p3-etapa-2-3j.md`; `ampliacion-probatoria-dubuissonhaiti-cap3-p3-etapa-2-3j.md`; `decision-academica-dubuissonhaiti-p3-etapa-2-3j.md`; `decision-normativa-sin-principal-con-dimensiones-p3-etapa-2-3j.md`.
